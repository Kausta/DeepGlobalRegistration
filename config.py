...
trainer_arg.add_argument('--trainer', type=str,
                         default='WeightedProcrustesTrainer')
...
trainer_arg.add_argument('--stat_freq', type=int,
                         default=40, help='Frequency for writing stats to log')
...
trainer_arg.add_argument('--clip_weight_thresh', type=float,
...
inlier_arg.add_argument('--inlier_use_direct_loss',
                        type=str2bool, default=True)

# Feature specific configurations
feat_arg = add_argument_group('feat')
feat_arg.add_argument('--feat_model', type=str, default='SimpleNetBN2C')
feat_arg.add_argument('--feat_model_n_out', type=int,
                      default=16, help='Feature dimension')
...
opt_arg.add_argument('--num_train_iter', type=int,
                     default=-1, help='train N iter if positive')
...
misc_arg.add_argument('--nn_max_n', type=int, default=250,
                      help='The maximum number of features to find nearest neighbors in batch')

# Dataset specific configurations
data_arg = add_argument_group('Data')
data_arg.add_argument('--dataset', type=str,
                      default='ThreeDMatchPairDataset03')
data_arg.add_argument('--voxel_size', type=float, default=0.025)
data_arg.add_argument('--threed_match_dir', type=str, default='.')
data_arg.add_argument('--kitti_dir', type=str, default=None,
                      help="Path to the KITTI odometry dataset. This path should contain <kitti_dir>/dataset/sequences.")
data_arg.add_argument('--kitti_max_time_diff', type=int, default=3,
...
eval_arg.add_argument('--success_rte_thresh', type=float,
                      default=0.3, help='Success if the RTE below this (m)')
eval_arg.add_argument('--success_rre_thresh', type=float,
                      default=15, help='Success if the RTE below this (degree)')
